...
import './Login.css';
export default function Signup({setpageRender}){
    const [getUserName,setUserName] = useState('');
    const [getPassword,setPassword] = useState('');
    const [conformgetPassword,conformsetPassword] = useState('');
    ...
    const handleSubmit=async(e)=>{
        ...
        if(!getUserName || !getPassword || !conformgetPassword){
            setError('Please fill all the fields');
            return ;
        }
        if(getPassword!==conformgetPassword){
            setError('Password do not match');
            return ;
        }
        if(!passwordRegex.test(getPassword)){
            setError("Password must contain 8 characters,contain at least one uppercase letter and one number");
            return;
        }
        try{
    ...
    return(
        <>
            <div className="Heading">
                <h1 className="HeadingText m-0">Signup</h1>
            ...
            <div className="FormConatiner d-flex flex-column align-items-center ">
                <form className="FormEleContainer d-flex flex-column gap-2" onSubmit={handleSubmit}>
                    ...
                    <div className="inputEle d-flex flex-column gap-1">
                        ...
                        <div className="PasswordInput d-flex flex-row">
                            <input className="pswEle" onChange={(ev) => conformsetPassword(ev.target.value)} placeholder='Enter Password' required type="password"/>
                            <div className="ShowandHideIconPswd d-flex flex-column justify-content-center align-items-center">
                                {
                                    toggleIcon2?
            ...
            Already have an account? <span className="textlink" onClick={()=>setpageRender('login')} style={{cursor:'pointer'}}>Login</span>
            </p>
