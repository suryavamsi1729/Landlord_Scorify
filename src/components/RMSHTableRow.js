...
export default function RMSHTableRow({ data, column, name }) {
    const color = (status) => {
        switch (status) {
            case "Completed":
                return {
    ...
    return (
        <tr className="tableRowContainer d-flex flex-row w-100">
            ...
            <td className={`${column[2].className} StatusColumn columndata d-flex flex-row align-items-center`}>
            ...
            <td className={`ActionColumn columndata d-flex flex-row align-items-center`}>
                <a href={data.Image[0]} target="_blank" rel="noopener noreferrer">
